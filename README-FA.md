...
└─────────┼─────────────────┼──────────────────┼──────────┘
     ...
     adg-sync.sh  (parallel nslookup)
          │
          ▼
     AdGuardHome REST API  ← /control/filtering/set_rules  (قوانین فیلترینگ سفارشی)
```

---

## 📖 سینتکس قوانین فیلترینگ DNS

این افزونه برای نوشتن قوانین بازنویسی (Rewrite) از سینتکس رسمی AdGuard استفاده می‌کند. برای جزئیات بیشتر به [مستندات رسمی سینتکس فیلترینگ AdGuard](https://adguard-dns.io/kb/general/dns-filtering-syntax/) مراجعه کنید.

قوانین به صورت زیر تولید می‌شوند:
- `! adg-sync` (خط کامنت)
- `||domain.com^$dnsrewrite=IP` (قانون بازنویسی)
...
curl -L -o /tmp/luci-app-adg-dnslookup.ipk https://github.com/mortezabahmani/luci-app-adg-dnslookup/releases/latest/download/luci-app-adg-dnslookup_3.1.7-1_all.ipk
...
curl -L -o /tmp/luci-app-adg-dnslookup.apk https://github.com/mortezabahmani/luci-app-adg-dnslookup/releases/latest/download/luci-app-adg-dnslookup_3.1.7-1_all.apk
